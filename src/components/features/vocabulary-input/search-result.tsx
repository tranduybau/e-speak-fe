...
export default function SearchResult(props: SearchResultProps) {
  ...
  return (
    <div
      className={` absolute top-[30px] z-10 w-full cursor-pointer space-y-2 rounded-md border border-border bg-black py-[10px]`}
    >
      {data.map((vocabulary, index) => (
        <button
          ...
          onClick={() => handleClickItem(vocabulary.text!)}
        >
          <h3 className="flex items-center font-semibold">
            <Search className="mr-2" /> {vocabulary.text}
          </h3>
        </button>
      ))}
